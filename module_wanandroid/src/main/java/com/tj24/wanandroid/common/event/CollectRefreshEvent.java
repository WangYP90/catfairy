...
public class CollectRefreshEvent extends BaseEvent {
    /**
     * 刷新类型  0 下拉刷新    1上拉加载
     */
    private int type;
    ...
    public static void  postRefreshEvent(int item){
        new CollectRefreshEvent(0,item).post();
    }

    public static void  postLoadMoreEvent(int item){
        new CollectRefreshEvent(1,item).post();
    }

    public CollectRefreshEvent(int type, int item) {
        this.type = type;
        this.item = item;
    }
